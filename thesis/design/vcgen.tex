...
\begin{prooftree}
  \AxiomC{$$}
  \LeftLabel{MOV-IMM}
  \UnaryInfC{$(P,\sigma) \vdash \mathrm{mov} ~ r_d, IMM \righthookarrow (P \implies r_d = IMM,\sigma[r_d \mapsto IMM])$}
\end{prooftree}

\begin{prooftree}
  \AxiomC{$$}
  \LeftLabel{MOV-REG}
  \UnaryInfC{$(P,\sigma) \vdash \mathrm{mov} ~ r_d, r_s \righthookarrow (P \implies r_d = r_s,\sigma[r_d \mapsto \sigma(r_s)])$}
\end{prooftree}
